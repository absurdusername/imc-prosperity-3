...
class DiffStrategy:
    def __init__(
        ...
        # NICE DATA-RELATED STUFF USED BY DA ALGORITHM!
        self.current_position = None
        self.daily_prices_and_volumes = [] # list of daily (mean price, total volume)
        self.rolling_prices = [] # list of daily volume-weight rolling-average price
        self.window = []

    def save(self):
        """Returns some JSON object.."""
        return {
            "daily_prices_and_volumes": self.daily_prices_and_volumes,
            "rolling_prices": self.rolling_prices,
        }

    def load(self, data):
        """Re-initializes all fields."""
        self.daily_prices_and_volumes = data["daily_prices_and_volumes"]
        self.rolling_prices = data["rolling_prices"]

    def parse_state(self, state: TradingState):
        """Computes all that nice data-related stuff used by da algorithm."""
        ...
        product_trades = own_trades.get(self.symbol, []) + market_trades.get(self.symbol, [])

        prices = [trade.price for trade in product_trades]
        quantities = [trade.quantity for trade in product_trades]
        
        volume = sum(quantities) 
        mean_price = np.average(prices, weights=quantities) if volume > 0 else self.default_value
        volume = max(volume, 1)
        
        self.daily_prices_and_volumes.append((mean_price, volume))
        if len(self.daily_prices_and_volumes) > self.window_size + self.lookback:
            self.daily_prices_and_volumes.pop(0)

        last_n_prices = [p for p, _ in self.daily_prices_and_volumes]
        last_n_volumes = [v for p, v in self.daily_prices_and_volumes]
        rolling_price = np.average(last_n_prices, weights=last_n_volumes)

        self.rolling_prices.append(rolling_price)
        if len(self.rolling_prices) > self.lookback + 1:
    ...
    def run(self, state: TradingState) -> list[Order]:
        ...
        if buy_limit > 0 and (soft_liquidate or hard_liquidate):
            ...
            price = true_value - 2 if soft_liquidate else true_value
...
class Trader:
    strategy_lookup = {
        "RAINFOREST_RESIN": DiffStrategy("RAINFOREST_RESIN", 50, default_value=10000, lookback=1),
        "KELP": DiffStrategy("KELP", 50, window_size=1, default_value=2000, lookback=10)}
