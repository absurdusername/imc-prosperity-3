...
class KelpStrategy(JmerleStrategy):
    def get_true_value(self):
        highest_bid = max(price for price, _ in self.outstanding_buy_orders)
        lowest_ask = min(price for price, _ in self.outstanding_sell_orders)
    ...
    def run(self, state: TradingState) -> list[Order]:
        ...
        max_bid = max(price for price, _ in self.outstanding_buy_orders)
        max_buy_price = max_bid - self.delta_buy if self.current_position > self.limit * self.tolerance else max_bid

        min_ask = min(price for price, _ in self.outstanding_sell_orders)
        min_sell_price = min_ask + self.delta_sell if self.current_position < self.limit * -self.tolerance else min_ask
...
"KELP": KelpStrategy("KELP", 50, window_size=1, tolerance=0, delta_buy=0, delta_sell=0)
    }
