...
class Status:
    ...
    def hist_maxamt_askprc(self, size: int) -> np.ndarray:
        ...
        prc_array = np.array([self.hist_order_depth('askprc', 1, size), self.hist_order_depth('askprc', 2, size),
                              self.hist_order_depth('askprc', 3, size)]).T
        amt_array = np.array([self.hist_order_depth('askamt', 1, size), self.hist_order_depth('askamt', 2, size),
                              self.hist_order_depth('askamt', 3, size)]).T
    ...
    def hist_maxamt_bidprc(self, size: int) -> np.ndarray:
        ...
        """
        res_array = np.empty(size)
        prc_array = np.array([self.hist_order_depth('bidprc', 1, size), self.hist_order_depth('bidprc', 2, size),
                              self.hist_order_depth('bidprc', 3, size)]).T
        amt_array = np.array([self.hist_order_depth('bidamt', 1, size), self.hist_order_depth('bidamt', 2, size),
                              self.hist_order_depth('bidamt', 3, size)]).T
    ...
    def hist_vwap_all(self, size: int) -> np.ndarray:
        ...
        for i in range(1, 4):
            ...
            if i == 0:
                ...
                res_array = res_array + np.where(bid_nan_idx, 0, tmp_bid_prc * tmp_bid_vol) + np.where(ask_nan_idx, 0,
                                                                                                       -tmp_ask_prc * tmp_ask_vol)
                volsum_array = volsum_array + np.where(bid_nan_idx, 0, tmp_bid_vol) - np.where(ask_nan_idx, 0,
                                                                                               tmp_ask_vol)
...
class Strategy:
    ...
    def mm_glft(
        ...
        delta_b = 1 / gamma * math.log(1 + gamma / kappa_b) + (
                    -mu / (gamma * sigma ** 2) + (2 * q + 1) / 2) * math.sqrt(
            (sigma ** 2 * gamma) / (2 * kappa_b * A_b) * (1 + gamma / kappa_b) ** (1 + kappa_b / gamma))
        delta_a = 1 / gamma * math.log(1 + gamma / kappa_a) + (mu / (gamma * sigma ** 2) - (2 * q - 1) / 2) * math.sqrt(
            (sigma ** 2 * gamma) / (2 * kappa_a * A_a) * (1 + gamma / kappa_a) ** (1 + kappa_a / gamma))
    ...
    def mm_ou(
        ...
        vfucn = lambda q, Q: -INF if (q == Q + 1 or q == -(Q + 1)) else math.log(
            math.sin(((q + Q + 1) * math.pi) / (2 * Q + 2)))
    ...
    def exchange_arb(state: Status, fair_price, next_price_move=0):
        ...
        while my_ask < fair_price:
            ...
            if my_ask > state.best_bid:
                ...
                expected_profit = (np.array(execution_prob_list) * (np.array(trading_profit_list) - cost) * np.array(
                    amount_list) / state.position_limit).sum()
...
class Trader:
    state_amethysts = Status('RAINFOREST_RESIN')
    state_starfruit = Status('KELP')
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        # round 1
        result["RAINFOREST_RESIN"] = Trade.amethysts(self.state_amethysts)
        result["KELP"] = Trade.starfruit(self.state_starfruit)
        ...
        #
        # # round 3
        # result["GIFT_BASKET"] = Trade.gift_basket(self.state_gift_basket, self.state_chocolate, self.state_strawberries,
        #                                           self.state_roses)
